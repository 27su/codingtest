//백준 : https://programmers.co.kr/learn/courses/30/lessons/72411
...
void comb(string s, int start, string made){
    if(made.size()>1){ // 2개 이상 주문된 것만
...
vector<string> solution(vector<string> orders, vector<int> course) {
    ...
    // 쿼리 수행
    for(int i:course)
        if(cnt[i]>1) // 길이가 i인 조합의 최대 주문 횟수가 2 이상인 경우만
